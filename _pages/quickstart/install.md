...
こちらは、[コマンドラインからインストールする](#コマンドラインからインストールする)とほぼ同じやり方になります。  違いとしては、実行するとEC-CUBEのインストール画面が立ち上がります。EC-CUBEのインストール画面から始めたい方におすすめです。
